...
$(function() {
    $("#table_cmd tbody").delegate(".listCmdNabaztag", 'click', function(event) {
        $('.description').hide();
        $('.version').hide();
        $('.required').hide();
        $('.description.' + $('#sel_addPreConfigCmdNabaztag').value()).show();
        $('.version.' + $('#sel_addPreConfigCmdNabaztag').value()).show();
        $('.required.' + $('#sel_addPreConfigCmdNabaztag').value()).show();
        $('#md_addPreConfigCmdNabaztag').modal('show');
        $('#bt_addPreConfigCmdNabaztagSave').undelegate().unbind();
        var tr = $(this).closest('tr');
        $("#div_mainContainer").delegate("#bt_addPreConfigCmdNabaztagSave", 'click', function(event) {
            var cmd_data = json_decode($('.json_cmd.' + $('#sel_addPreConfigCmdNabaztag').value()).value());
            tr.setValues(cmd_data, '.cmdAttr');
            $('#md_addPreConfigCmdNabaztag').modal('hide');
        });
    });

    $("#sel_addPreConfigCmdNabaztag").on('change', function(event) {
        $('.description').hide();
        $('.version').hide();
        $('.required').hide();
